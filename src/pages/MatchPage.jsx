...
export default function MatchPage() {
  ...
  const submitResult = async (winnerTeam) => {
    ...
    setSubmitting(true);
    try {
      await matchesApi.submitResult(id, winnerTeam);
      toast.success('Résultat soumis !');
      load();
    ...
    <div className="max-w-4xl mx-auto px-4 py-10 animate-fade-in">
      ...
      {match.status === 'IN_PROGRESS' && isParticipant && (
        <div className="card p-5 mb-4">
          <h2 className="font-bold mb-4">🏆 Soumettre le résultat</h2>
          <div className="flex gap-4 flex-wrap">
            <button onClick={() => submitResult(1)} disabled={submitting} className="btn-primary flex-1">Victoire Équipe 1</button>
            <button onClick={() => submitResult(2)} disabled={submitting} className="btn-secondary flex-1">Victoire Équipe 2</button>
          </div>
          <button onClick={() => setShowDisputeForm(!showDisputeForm)} className="text-sm text-gray-500 hover:text-yellow-500 mt-3 transition-colors">
            ⚠️ Signaler un problème
          </button>
          {showDisputeForm && (
            <div className="mt-3 space-y-3">
              <textarea value={disputeDesc} onChange={e => setDisputeDesc(e.target.value)}
                className="input resize-none h-24" placeholder="Décrivez le problème..." />
