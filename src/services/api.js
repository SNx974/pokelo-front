...
api.interceptors.response.use(
  ...
  (err) => {
    ...
    if (status && status >= 400 && status !== 401) {
      ...
      toast.error('Impossible de joindre le serveur. Vérifie ta connexion.');
    }
...
export const teamsApi = {
  ...
  onlineStatus:      (id)                  => api.get(`/teams/${id}/online-status`),
...
export const matchesApi = {
  ...
  getChatMessages: (id)          => api.get(`/matches/${id}/chat`),
  sendChatMessage: (id, content) => api.post(`/matches/${id}/chat`, { content }),
};
...
export const matchmakingApi = {
  ...
  activeMatch: ()     => api.get('/matchmaking/active-match'),
};
